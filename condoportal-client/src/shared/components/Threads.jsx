// Fondo animado del hero — diseño de Frontend 1.
//
// Dibuja con WebGL una maraña de hilos que ondulan con ruido Perlin y siguen
// al ratón. Se usa sólo como decoración: va marcado aria-hidden por quien lo
// monta y, si la persona pidió menos movimiento, ni siquiera se crea el
// contexto de WebGL.
//
// Adaptado: el color entra por token (el original recibía un array fijo) y se
// respeta `prefers-reduced-motion`. Los estilos viven en styles/animaciones.css.
...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }) => {
  ...
  // Las props vivas se guardan en un ref para que cambiarlas sólo mueva los
  // uniforms del shader, sin rehacer el contexto de WebGL. La copia se hace en
  // un efecto y no durante el render: escribir en un ref mientras se renderiza
  // rompe las reglas de React (y lo marca el linter).
  ...
  useEffect(() => {
    ...
    // Shader a pantalla completa y en bucle: es justo lo que sobra cuando
    // alguien pide menos movimiento. El hero se ve igual, sólo sin hilos.
